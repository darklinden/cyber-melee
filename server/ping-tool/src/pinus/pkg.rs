...
impl Pkg {
    ...
    pub fn encode(&self) -> Result<Vec<u8>> {
        ...
        let body = match content {
            PkgBody::None => None,
            PkgBody::Msg(msg) => Some(msg.encode()?),
            PkgBody::StrMsg(str_msg) => Some(str_msg.as_bytes().to_vec()),
        };

        let length = if let Some(b) = body.as_ref() {
            b.len()
        } else {
            0
        };
        ...
        while offset < bytes.len() {
            ...
            if pkg_type == PkgType::Data as u8 {
                ...
                rs.push(Pkg {
                    pkg_type: pkg_type.into(),
                    content: match body {
                        Some(b) => PkgBody::StrMsg(String::from_utf8(b).unwrap()),
                        None => PkgBody::None,
                    },
                });
            }
