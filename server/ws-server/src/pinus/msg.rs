...
impl Msg {
    ...
    fn calculate_msg_id_bytes(&self) -> usize {
        if !self.has_id() {
            return 0;
        }
        let id = self.id;
    ...
    fn encode_msg_flag(&self, buffer: &mut [u8], offset: usize) -> Result<usize> {
    ...
    fn encode_msg_route(&self, buffer: &mut [u8], offset: usize) -> Result<usize> {
        if self.has_route() {
            if self.route.code.is_some() {
                ...
                Ok(offset + 2)
            } else if self.route.name.is_some() {
    ...
    pub fn encode(&self) -> Result<Vec<u8>> {
        ...
        let id_bytes = self.calculate_msg_id_bytes();
    ...
    pub fn decode(buffer: &[u8]) -> Result<Msg> {
        ...
        if Self::msg_has_route(msg_type) {
            if compress_route != 0 {
                ...
                let route_code = u16::from_be_bytes(route_bytes);
                let route_name = if cfg!(debug_assertions) {
                    code2route(route_code)
                } else {
                    None
                };
                route = Route {
                    code: Some(route_code),
                    name: route_name,
                };
                offset += 2;
            } else {
