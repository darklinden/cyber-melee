...
export async function parseJsonFieldBaseType(
    typeStr: string,
    str: string,
    obj: {
        enums: { [key: string]: IPathStruct },
        ...
        if (isNaN(+str)) {
            ...
            if (str.includes('::')) {
                ...
                if (arr.length == 2) {
                    ...
                    if (obj.enums.hasOwnProperty(enumName)) {

                        let enumKV: { [key: string]: number } = {};
                        let enumVK: { [key: number]: string } = {};

                        let rows = await CsvUtil.loadDataSync(obj.enums[enumName].csv);
                        for (const row of rows) {
                            if (row[0].startsWith('#')) {
                                // 跳过表头和注释
                                continue;
                            }
                            enumKV[row[0]] = parseInt(row[1]);
                            enumVK[parseInt(row[1])] = row[0];
                        }

                        if (enumKV.hasOwnProperty(enumValue)) {
                            return enumKV[enumValue];
                        }
                        else if (!enumValue || !enumValue.length) {
                            return 0;
                        }
                        else {
                            throw new Error('未知枚举值: ' + enumName + ' - ' + JSON.stringify(enumKV) + ' - [' + enumValue + ']');
                        }
                    }
...
export async function parseJsonFieldStructType(
    ...
    for (let i = 0; i < struct.subStructs.length; i++) {
        let item = items[i];
        const { struct_name, type_value } = await parseJsonField(struct.subStructs[i], item, obj);
        if (type_value != null)
            o[struct_name] = type_value;
    }
    return o;
}

// key value
export async function parseJsonField(
    struct: JsonFieldStruct,
    str: string,
    obj: {
        enums: { [key: string]: IPathStruct },
        structs: { [key: string]: IPathStruct },
        tables: { [key: string]: IPathStruct }
    }): Promise<{ struct_name: string, type_value: any }> {
    let value: any = null;
    ...
    if (struct.fieldType == FieldType.Base) {
        if (struct.isArr) {
            ...
            for (const item of arr) {
                value.push(await parseJsonFieldBaseType(struct.typeStr, item, obj));
            }
        }
        else {
            if (str == null || str.trim() == '') {
            ...
            value = await parseJsonFieldBaseType(struct.typeStr, str, obj);
        }
        ...
        if (struct.isArr) {
            ...
            value = [];
            for (const item of arr) {
                var o = await parseJsonFieldStructType(struct, item, obj)
                o && value.push(o);
            }
        }
        else {
            value = await parseJsonFieldStructType(struct, str, obj);
        }
        return { struct_name: struct.name, type_value: value };
    }
    else {
...
async function structMatchRow(
    ...
    for (const row of dataRows) {
        ...
        for (let i = 0; i < struct.length; i++) {
            ...
            const { struct_name, type_value } = await parseJsonField(field, value, obj);
            if (type_value != null)
                o[struct_name] = type_value;
        }
        result.rows.push(o);
...
export async function generateJson(obj: {
    ...
    for (const table in obj.tables) {
        ...
        const tableData = await structMatchRow(struct, dataRows, obj);
